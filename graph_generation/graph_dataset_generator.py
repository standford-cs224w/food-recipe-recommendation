...
def generate_graph(user_mapping: list[tuple[int, int]], recipe_x: torch.Tensor, edge_index: torch.Tensor, edge_label: Optional[torch.Tensor]) -> HeteroData:
    ...
    print('--Graph before ToUndirected: \n', graph_data)
...
if __name__ == '__main__':
    ...
    _, user_mapping = load_nodes_from_csv(
        f'{base_data_path}/interactions_train.csv', 
        index_col='user_id'
    )

    print('Loading recipe nodes...')
    ...
    for data_split_name in ['train', 'validation', 'test']:
        print(f'Generating {data_split_name} graph...')
        print('-Loading edges...')
        rating_path = f'{base_data_path}/interactions_{data_split_name}.csv'
        ...
        graph_path = f'{base_data_path}/graph/v{graph_version}/{data_split_name}_graph.pt'
        store_graph(graph, graph_path)
        print(f'{data_split_name} graph stored in {graph_path}.')
